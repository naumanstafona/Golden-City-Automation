...
export class CommonSteps {
  ...
  protected readonly timeout_small: number = 50000;
  ...
  async waitingForURL(expectedUrl: string) {
    try {
      console.log(
        `[${new Date().toISOString()}] Waiting for URL: ${expectedUrl}`
      );
  ...
  async waitForButton(buttonName: string) {
    try {
      ...
      await this.page
        ...
        .waitFor({ state: "visible", timeout: this.timeout_large });
    } catch (error) {
  ...
  async waitForPlaceholder(placeHolderName: string) {
    try {
      ...
      await this.page
        .getByPlaceholder(placeHolderName)
        .waitFor({ state: "visible", timeout: this.timeout_small });
    } catch (error) {
  ...
  async fillingPlaceholder(placeHolderName: string, expectedName: string) {
    try {
      console.log(
        `Filling placeholder: ${placeHolderName} with ${expectedName}`
      );
      await this.page.getByPlaceholder(placeHolderName).fill(expectedName);
    } catch (error) {
      console.error(
        `Error filling placeholder ${placeHolderName} with ${expectedName}:`,
        error
      );
      process.exit(1);
    }
  ...
  async waitforLocatorByLabel(locatorName: string) {
    try {
      console.log(`Waiting For :${locatorName} by label`);
      await this.page
        .getByLabel(locatorName)
        .waitFor({ state: "visible", timeout: this.timeout_large });
    } catch (error) {
      console.error("Error Waiting For :${locatorName} by label", error);
      process.exit(1);
    }
  }

  async fillingLocatorByLabel(locatorName: string, value: string) {
    try {
      console.log(`Filling :${locatorName} with :${value} label`);
      await this.page.getByLabel(locatorName).fill(value);
    } catch (error) {
      console.error(
        "Error Filling :${locatorName} with :${value} label",
        error
      );
      process.exit(1);
    }
  ...
  async waitForLinkButton(buttonName: string) {
    try {
      console.log(`Waiting for link button: ${buttonName}`);
      await this.page
        .getByRole("link", { name: buttonName })
        .waitFor({ state: "visible", timeout: this.timeout_large });
    } catch (error) {
      console.error(`Error waiting for button ${buttonName}:`, error);
      process.exit(1);
  ...
  async selectingDropdownValueByClicking(fieldLabel: string, value: string) {
    const dropdown = this.page.getByLabel(fieldLabel).locator("span");
  ...
  async waitForGridCell(text: string) {
    try {
      console.log(`Waiting for GridCell with name: ${text}`);
      await this.page
        .getByRole("gridcell", { name: text })
        .waitFor({ state: "visible", timeout: this.timeout_large });
    } catch (error) {
      console.error(`Waiting for GridCell with namen ${text}:`, error);
      process.exit(1);
    }
  }
  ...
  async waitingForCellHavingTextforVerification(text: string) {
    try {
      ...
      console.error(
        `Error waiting for  Email Domain Placeholder ${text}:`,
        error
      );
      process.exit(1);
    }
  ...
  async waitForTitle(titleName: string) {
    try {
      ...
      await this.page
        .getByTitle(titleName, { exact: true })
        .waitFor({ state: "visible", timeout: this.timeout_large });
    } catch (error) {
  ...
  async fillingLocator(locatorName: string, expectedValue: string) {
    try {
      console.log(
        `Filling locator: ${locatorName} with value: ${expectedValue}:`
      );
      await this.page.locator(locatorName).fill(expectedValue);
    } catch (error) {
      console.error(
        `Error in Filling locator: ${locatorName}: with value:${expectedValue}`,
        error
      );
      process.exit(1);
    }
  ...
  async waitForElementByTextWithin(selector: string, textName: string) {
    try {
      console.log(
        `Waiting for text: '${textName}' within element: '${selector}'`
      );
      ...
      console.error(
        `Error waiting for text '${textName}' within element '${selector}':`,
        error
      );
      process.exit(1);
    }
  ...
  async clickElementByTextWithin(selector: string, textName: string) {
    try {
      ...
      console.error(
        `Error clicking text '${textName}' within element '${selector}':`,
        error
      );
      process.exit(1);
    }
  ...
  async handleAndAcceptDialog(triggerSelector: string) {
    ...
    try {
      console.log(
        `Clicking on element: '${triggerSelector}' to trigger dialog`
      );
  ...
  async fillingLocatorbyGettingRoleTextboxandPressingEnter(
    locatorName: string,
    expectedValue: string
  ) {
    try {
      console.log(
        `Filling locator by Getting Role Textbox: ${locatorName} with value: ${expectedValue}`
      );
      await this.page
        .locator(locatorName)
        .getByRole("textbox")
  ...
  async waitForElementToBeVisibleinCell(name: string, exact: boolean = true) {
    try {
      console.log(
        `Waiting for element with role: cell, name: ${name}, exact: ${exact} to be visible.`
      );
      await this.page
        .getByRole("cell", { name: name, exact: exact })
        .waitFor({ state: "visible", timeout: this.timeout_large });
      console.log(
        `Element with role: cell, name: ${name}, exact: ${exact} is visible.`
      );
    } catch (error) {
      console.error(
        `Error waiting for element with role: cell, name: ${name}, exact: ${exact} to be visible.`,
        error
      );
      process.exit(1);
    }
  ...
  async reloadPage() {
    try {
      ...
      console.error("Error while reloading the page:", error);
      process.exit(1);
    }
  }
  ...
  async generateRandomUser(): Promise<{ name: string; email: string }> {
    const randomString: string = Math.random().toString(10).substring(2, 10);
    const name: string = `User_${randomString}`;
    const email: string = `nauman.umer+${name}@jammber.com`;
  ...
  async removeTextFromField(locator: string) {
